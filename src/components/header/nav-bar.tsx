...
const NavLink = ({ href, isCurrent, Icon, children }: NavLinkProps) => {
  return (
    <Link
      ...
      className={cn(
        "group flex gap-2 items-center text-sm rounded-full text-blue-500 py-2 px-4 transition-all duration-300",
        {
          "bg-blue-500 border text-white font-semibold": isCurrent,
...
const NavBar = () => {
  ...
  return (
    <nav className="hidden sm:flex items-center gap-4 ">
      <NavLink href="/" Icon={MessageCircle} isCurrent={pathName === "/"}>
        Chat
      </NavLink>
      <NavLink
        href="/friends"
